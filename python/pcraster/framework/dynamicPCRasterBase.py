...
class DynamicModel(dynamicBase.DynamicBase):
  ...
  def report(self,
    variable,
    name, directoryPrefix = "", lisem=False):
    """
    ...
    `directoryPrefix`
     States the location to store the map
     
    `lisem`
     Boolean for whether this is a LISEM run and the directoryPrefix 
     should be concatenated with the name.

    """
    self._reportNew(variable, name, directoryPrefix=directoryPrefix, lisem=lisem)
